...
class AdmiralsportsSpider(BaseSpider):
    ...
    allowed_domains = ["www.admiralsports.shop", "payment.admiralsports.shop/graphql"]
    ...
    def parse_list(self, response: TextResponse):
        ...
        if 'dl' in meta:
            ...
            dl = {'TotalCount': total_count, 'TotalPage': total_page, 'Url': meta['list_url'], 'PageIndex': page, 'ProductList': prods, 'NextPageUrl': nextPageUrl, 'FromKey':FromPage.FROM_PAGE_PRODUCT_LIST}
    ...
    def get_list_url(self, page_index: int, page_size: int, category_id: int) -> str:
        ...
        """
        # 使用字符串格式化替换变量
        querystr = template.replace("{category_id}", category_id).replace("{page_size}", str(page_size)).replace("{current_page}", str(page_index))
